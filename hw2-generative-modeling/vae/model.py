...
class Encoder(nn.Module):
    ...
    def __init__(self, input_shape, latent_dim):
        ...
        in_shape = self.convs(torch.ones(input_shape).unsqueeze(dim=0))
        self.fc = nn.Linear(in_features=int(np.prod(in_shape.shape[1:])), out_features=latent_dim)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
    ...
    def forward(self, x):
        ...
        ##################################################################
        return self.fc(self.convs(x).flatten(start_dim=1))
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class VAEEncoder(Encoder):
    def __init__(self, input_shape, latent_dim):
        ...
        ##################################################################
        in_shape = self.convs(torch.ones(input_shape).unsqueeze(dim=0))
        self.fc = nn.Linear(int(np.prod(in_shape.shape[1:])), 2*latent_dim)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Decoder(nn.Module):
    ...
    def __init__(self, latent_dim, output_shape):
        ...
        self.fc = nn.Linear(in_features=self.latent_dim, out_features=np.prod(self.base_size[:]))
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
