...
def get_optimizers_and_schedulers(gen, disc):
    ...
    # 100K iterations.
    ##################################################################
    scheduler_discriminator = torch.optim.lr_scheduler.LambdaLR(optim_discriminator, lr_lambda=lambda step: max(0, 1-step/500_000))
    scheduler_generator = torch.optim.lr_scheduler.LambdaLR(optim_generator, lr_lambda=lambda step: max(0, 1-step/500_000))
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
    return (
    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                # discriminator on it.
                ###################################################################
                alpha = torch.randn(train_batch.size(0), 1, 1, 1, device=train_batch.device)
            ...
            if iters % log_period == 0 and iters != 0:
                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled=amp_enabled):
                        ...
                        ##################################################################
                        z = torch.randn(100, 128, device=next(gen.parameters()).device)
                        generated_samples = (gen.forward_given_samples(z) +1)/2
                        ##################################################################
                        #                          END OF YOUR CODE                      #
                        ##################################################################
                    save_image(
