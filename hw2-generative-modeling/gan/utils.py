...
def interpolate_latent_space(gen, path):
    ...
    ##################################################################
    z_lin = torch.linspace(-1, 1, 10)
    latent_space=[]
    for i in range(10):
        for j in range(10):
            z = torch.zeros(128, device=next(gen.parameters()).device)
            z[0], z[1] = z_lin[i], z_lin[j]
            latent_space.append(z)
    latent_space = torch.stack(latent_space, dim=0)
    image = (gen.forward_given_samples(latent_space)+1)/2
    save_image(image, path, nrow=10)
