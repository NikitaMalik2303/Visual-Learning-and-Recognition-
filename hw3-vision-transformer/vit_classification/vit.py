...
class ViT(nn.Module):
    ...
    def __init__(self, patch_dim, d_model, d_ff, num_heads, num_layers, num_patches, num_classes, device = 'cuda'):
        ...
        self.patch_embedding = nn.Linear(3*patch_dim*patch_dim, d_model) # TODO (Linear Layer that takes as input a patch and outputs a d_model dimensional vector)
    ...
    def forward(self, images):
        ...
        patches_embedded = self.patch_embedding(patches) #shape=(N, num_patches, d_model)
        
        cls_token = self.cls_token #shape=(1, 1, d_model)
        cls_token = cls_token.expand(patches_embedded.shape[0], -1, -1) #shape=(N, 1, d_model)
        output = torch.concat((cls_token, patches_embedded), dim=1) # TODO (append a CLS token to the beginning of the sequence of patch embeddings)
