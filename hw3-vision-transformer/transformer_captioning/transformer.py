...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = (query@key.transpose(-1, -2))/math.sqrt(D//H) #shape = (N, H, S, T)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        output = x + self.encoding(positions.unsqueeze(0))
...
class TransformerDecoder(nn.Module):
    ...
    def get_causal_mask(self, _len):
        ...
        mat = torch.ones(_len, _len)
        mask = torch.tril(mat).to(torch.bool)
        return mask
